...
Implemented:
1. DTOs with validation (src/modules/auth/dto/)
register.dto.ts — Email, password (min 8 chars), optional name
login.dto.ts — Email and password validation
refresh.dto.ts — Placeholder for refresh requests (uses cookie)
...
3. Passport strategies (src/modules/auth/strategies/)
jwt.strategy.ts — Validates JWT access tokens from Authorization header
refresh.strategy.ts — Validates refresh tokens from cookies (for future use)
4. Guards and decorators (src/modules/auth/guards/ & decorators/)
Guards:
jwt-auth.guard.ts — Protects routes, checks @Public() decorator
roles.guard.ts — Enforces RBAC based on @Roles() decorator
Decorators:
...
5. AuthController (src/modules/auth/auth.controller.ts)
Endpoints:
POST /auth/register — Public, registers user, returns tokens
POST /auth/login — Public, authenticates user, returns tokens
POST /auth/refresh — Public, refreshes access token, rotates refresh token
POST /auth/logout — Protected, invalidates refresh token
...
Configures JwtModule with dynamic secrets
Registers Passport strategies
Exports guards for use in other modules
Integrates with global services (Prisma, Config, Logger)
File structure
src/modules/auth/├── dto/│   ├── register.dto.ts│   ├── login.dto.ts│   └── refresh.dto.ts├── strategies/│   ├── jwt.strategy.ts│   └── refresh.strategy.ts├── guards/│   ├── jwt-auth.guard.ts│   └── roles.guard.ts├── decorators/│   ├── public.decorator.ts│   ├── roles.decorator.ts│   └── current-user.decorator.ts├── auth.controller.ts├── auth.service.ts└── auth.module.ts
Build status
